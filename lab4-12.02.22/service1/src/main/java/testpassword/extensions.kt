...
fun <T> Iterable<T>.paginate(limit: Int?, offset: Int?): List<T> =
    try {
        if (limit == 0 && offset == 0 || limit == null && offset == null) this.toList()
        ...
        else if (limit == 0) emptyList()
        else this.toList().let { it.subList(offset, if (limit != 0) offset + limit else it.count() - 1) }
